...
- "ADCIRC config" / "fort.15" / "fort.14" → call adcirc_parse_fort15(...), adcirc_explain_parameter(...)
- "SCHISM config" / "param.nml" / "hgrid" → call schism_parse_param_nml(...), schism_explain_parameter(...)
- "GOES satellite image" → call goes_get_latest_image(...), goes_get_sector_image(...)
- "streamflow" / "river discharge" / "USGS gauge" → call usgs_get_instantaneous_values(...), usgs_get_flood_status(...)
- "wind observations" / "weather station" → call winds_get_latest_observation(...), winds_find_nearest_stations(...)
- "waves" / "buoy" / "WW3 forecast" → call ww3_get_buoy_observations(...), ww3_get_forecast_at_point(...)
- "source code" / "subroutine" / "documentation" → call search_documentation(query=...)
